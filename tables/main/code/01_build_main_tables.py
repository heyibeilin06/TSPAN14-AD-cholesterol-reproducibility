#!/usr/bin/env python3
"""Rebuild the four main manuscript tables as compact SCI three-line tables."""
...
def citation_audit(doc):
    first_positions = {}
    all_hits = {str(i): [] for i in range(1, 5)}
    ...
    if sorted(first_positions) != [1, 2, 3, 4]:
        raise RuntimeError(f"Missing in-text table citation: {first_positions}")
    order = [first_positions[i] for i in range(1, 5)]
...
def main():
    ...
    if len(doc.tables) != 4:
        raise RuntimeError(f"Expected four source tables, found {len(doc.tables)}")
    expected_headers = ["Analysis layer", "Evidence layer", "Analysis", "Evidence domain"]
