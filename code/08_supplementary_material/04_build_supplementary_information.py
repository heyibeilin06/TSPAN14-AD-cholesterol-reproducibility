...
FIG_LEGENDS = [
    ...
    "Supplementary Figure S3 | Colocalization and fine-mapping diagnostics at TSPAN14. A, Trait-specific posterior inclusion probabilities for the 15 highest-ranked variants. B, Sizes and top posterior inclusion probabilities of SuSiE credible sets. C, Colocalization posterior probabilities for the exact exon5-6 event with AD and cholesterol traits; the axis is restricted to the high-posterior range to resolve differences among models.",
...
def build():
    ...
    for name,size,color in (("Heading 1",15,"17365D"),("Heading 2",11,"17365D")):
        styles[name].font.name="Arial"; styles[name].font.size=Pt(size); styles[name].font.bold=True; styles[name].font.color.rgb=RGBColor.from_string(color)
    cover=doc.add_paragraph(); cover.alignment=WD_ALIGN_PARAGRAPH.CENTER; cover.paragraph_format.space_before=Pt(70)
    run=cover.add_run(TITLE); run.font.name="Arial"; run.font.size=Pt(19); run.bold=True; run.font.color.rgb=RGBColor.from_string("17365D")
    sub=doc.add_paragraph(); sub.alignment=WD_ALIGN_PARAGRAPH.CENTER; sub.paragraph_format.space_before=Pt(18)
    r=sub.add_run("Supplementary Information"); r.font.name="Arial"; r.font.size=Pt(15); r.bold=True
